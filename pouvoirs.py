    ...
    for Z in range(-1,4):
        ...
        for Y in range(-2,4):
            ...
            for X in range(-3,2):
                ...
                if(blockType!=57):
                    mc.setBlock(pos.x,1+X,pos.y,+Y,pos.z-Z,85)
    ...
    for Z in range(0,3):
        ...
        for Y in range(-1,6):
            ...
            for X in range(-2,1):
                ...
                if(blockType!=57):
                    mc.setBlock(round(pos.x)+X,round(pos.y)+Y,round(pos.z)-Z,0)
...
def BP_superPouvoir(player) :
    ...
    for Z in range(1,4):
        ...
        for X in range(-1,2):
            for Y in [0,4]:
                #On recupere le type de bloc sur la position sur laquelle on creuse
                blockType = mc.getBlock(round(int(pos.x))+X-1,round(int(pos.y)-1+Y),round(int(pos.z))-Z+1)
                        
                #Si le bloc en question est un bloc de diamant,on ne doit pas le supprimer
                #Sinon on peut le retirer en le remplacant par de l'air
                if(blockType!=57):
                    mc.setBlock(round(int(pos.x))+X-1,round(int(pos.y)-1+Y),round(int(pos.z))-Z+1,49)
    ...
    for Z in [1,3,1,3]:
        ...
        for X in [-1,1,-1,1]:
            for Y in range(0,3):
                #On recupere le type de bloc sur la position sur laquelle on creuse
                blockType = mc.getBlock(round(int(pos.x))+X-1,round(int(pos.y))+Y,round(int(pos.z))-Z+1)
                    
                #Si le bloc en question est un bloc de diamant,on ne doit pas le supprimer
                #Sinon on peut le retirer en le remplacant par de l'air
                if(blockType!=57):
                    mc.setBlock(round(int(pos.x))+X,round(int(pos.y))+Y,round(int(pos.z))-Z+1,49)
